...
typedef struct TellerQueue {
	int status; // status, 0: close, 1:?u??U?@????A?Y?N close, 2: open
	int front; // front pointer?A???V??@?? element ?? index
	int rear; // rear pointer?A???V???@?? element ???U?@?? index
	int count; // queue ???b??????H??
	int current_served_time; // ??e?b?B?z?~?????H??B?z??????U?????
